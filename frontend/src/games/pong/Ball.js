...
class Ball {
  ...
  serve(net, sign) {
    ...
    if (
      ...
      if (this.audio && this.scoreSound && !this.scoreSound.isPlaying) {
  ...
  update(
    ...
    if (this.y < -50 && this.sendLock === false && this.serving === false) {
      ...
      if (this.isServerDemon) {
        this.serverWin++;
        if (this.serverWin === 3) {
          if (this.audio && this.Achievement && !this.Achievement.isPlaying) {
  ...
  audioLoader(sm) {
    if (!this.audio) {
      ...
      sm.audioLoader.load(
        `https://${window.location.hostname}:${window.location.port}/games/Sounds/AchievementSong.mp3`,
        (buffer) => {
