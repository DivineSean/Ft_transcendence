...
export class Table {
  ...
  async render() {
    this.table = await this.loader
      .loadAsync(
        `https://${window.location.hostname}:3000/src/games/pong/table.glb`,
      )
    ...
    this.midtable = await this.loader
      .loadAsync(
        `https://${window.location.hostname}:3000/src/games/pong/midtable.glb`,
      )
      .then((data) => data.scene.children[0]);
    ...
    this.endtable = await this.loader
      .loadAsync(
        `https://${window.location.hostname}:3000/src/games/pong/endtable.glb`,
      )
      .then((data) => data.scene.children[0]);
