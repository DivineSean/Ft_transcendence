    ...
    return (
        <>
            <div className="bg-[#165044] w-full items-center flex gap-16">
                ...
                <button onClick={() => resetRoute('Games')} className="font-press-start text-txt-xs md:text-txt-md">Games</button>
                {
                    routes.map((route, index) => (
                        <React.Fragment key={index}>
                            {index < routes.length && <h2 className="font-press-start text-txt-xs md:text-txt-md"> {'>'} </h2>}
                            <button onClick={() => resetRoute(index)} className="font-press-start text-txt-xs md:text-txt-md">{route}</button>
                        </React.Fragment>
                    ))
                }
            </div>
