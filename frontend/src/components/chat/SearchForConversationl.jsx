...
const SearchForConversation = ({setDisplaySearch}) => {
	...
	const searchConversations = async () => {
		try {
			const res = await FetchData.get(`chat/conversations/search/?query=${searchQuery}`);
			if (res.ok) {
	...
	return (
		<>
			...
			<div className="p-16 hover-secondary rounded-lg backdrop-blur-3xl border-[0.5px] border-stroke-sc md:w-[600px] w-[90%]
					...
					>
						...
						foundConversations.map((conversation) => (
							<li
								...
								onClick={() => {navigate(`/chat/${conversation.ConversationId}`); setDisplaySearch(false)}}
							...
							>
								...
								<section className="flex flex-col justify-center normal-case">
									<h1 className="tracking-wider text-txt-sm font-semibold">{conversation.user.first_name} {conversation.user.last_name}</h1>
									<p className="text-txt-xs text-stroke-sc tracking-wide">see the conversation</p>
								</section>
							</li>
						))
						:
						<p className="py-16 flex justify-center text-txt-sm text-stroke-sc">no conversation founded</p>
						}
					</ul>
				</div>
			</div>
