...
curl -s -X PUT "https://elasticsearch:9200/_index_template/logs-template" \
	...
	-H "Content-Type: application/json" \
	-d '{
        "index_patterns": ["nginx-access-*", "nginx-error-*", "uvicorn-*"],
        "template": {
          "settings": {
...
echo "Importing Kibana Configuration/Dashboards"
curl -s -k \
	-X POST "https://kibana:5601/api/saved_objects/_import?overwrite=true" \
	-u "${ELASTIC_USERNAME}:${ELASTIC_PASSWORD}" \
	-H "kbn-xsrf: true" \
	-H "Content-Type: multipart/form-data" \
	--form file=@/usr/share/elasticsearch/config/kibana_config.ndjson |
	grep -q '"success":true' || exit 1

echo "All done!, removing the setup container"
# INFO: this might fail if the host doesn´t have permissions (eg: Linux Clusters)
CONTAINER_ID=$(hostname)
curl -s \
	-X DELETE "http://localhost/containers/${CONTAINER_ID}?force=true" \
	--unix-socket /var/run/docker.sock || true
