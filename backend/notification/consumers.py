...
class ChatAndNotificationConsumer(WebsocketConsumer):
    ...
    def disconnect(self, code):
        ...
        async_to_sync(self.channel_layer.group_discard)(self.notif_room, self.channel_name)
    ...
    def receive(self, text_data):
        try:
            ...
            elif message_type in ["message", "read", "typing", "stopTyping"]: # hado li kaynin for now ? 
    ...
    def chat_message(self, event):
        self.send(text_data=json.dumps({
            ...
            "firstName": self.user["first_name"] if event["sender"]["id"] != self.user.id 
                        else event["sender"]["first_name"],
            "lastName": self.user["last_name"] if event["sender"]["id"] != self.user.id 
