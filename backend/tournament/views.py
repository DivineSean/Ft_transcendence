...
def CreateTournament(request):
    ...
    existingLobby = Lobby.objects.filter(creator=request._user, isCompleted=False).first()
...
def listTournaments(request):
    tournaments = Lobby.objects.filter(isCompleted=False).values('lobbyID', 'maxPlayers')
