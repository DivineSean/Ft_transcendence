...
def CreateTournament(request):
    ...
    try:
        newLobby = Tournament.objects.create(creator=request._user, maxPlayers=maxPlayers, total_rounds=int(math.log2(maxPlayers)), game = Game.objects.get(name = "pong"))
    except:
        return Response({"message" : "Probably pong game doesn't exists"}, status = 400)
    newLobby.addPlayer(request._user)

    return Response(
        {"message": "Tournament created successfully", "lobbyID": str(newLobby.lobbyID)},
        status=201,
    )
